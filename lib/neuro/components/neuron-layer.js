...
class NeuronLayer {
    constructor(network, options) { 
        ...
        this.app = options.app || network.app;
        ...
        if (options.neuronCount > 0) { 
            ...
            for (let i = 0; i < options.neuronCount; i++) { 
                const n = new Neuron(this, { index: i, isBias: (i >= biasIndex), vectorHandler: options.vectorHandler });
                this.neurons.push(n);
    ...
    setLayout(layerWidth = -1, layerHeight = -1) { 
        ...
        for (let i = 0; i < this.neurons.length; i++) { 
            ...
            n.vectorHandler.setValues(n.agent.position, x, y);
        }
